...
# ------------------------------------------------------------------
    ...
    st.title("🎯 4% 法則：財富自由與退休試算")
    st.markdown("> **4% 法則定義**：當你的資產達到年支出的 25 倍時，每年提取 4% 資金，在歷史數據中極高機率可支撐 30 年以上的退休生活。")

    col_sim_in, col_sim_out = st.columns([1, 2])
    
    with col_sim_in:
        st.subheader("⚙️ 退休生活設定")
        monthly_expense = st.number_input("退休後每月生活預算 (萬)", value=10.0, step=0.5)
        # 計算退休目標金額 (年支出 * 25)
        fire_target = monthly_expense * 12 * 25
        st.success(f"📌 您的 4% 法則目標金額：**{fire_target:,.0f} 萬**")
        
        st.markdown("---")
        st.subheader("📈 累積期參數模擬")
        init_capital = st.number_input("模擬啟始本金 (萬)", value=100.0)
        monthly_save = st.number_input("模擬每月加碼 (萬)", value=5.0)
        roi_annual = st.slider("預期年化報酬率 (%)", 0, 20, 8) / 100
    ...
    with col_sim_out:
        ...
        for m in range(months_sim + 1):
            val_sim = init_capital * ((1 + roi_annual/12)**m) + (monthly_save * (((1 + roi_annual/12)**m - 1) / (roi_annual/12)))
            sim_data_list.append({"月份": m, "資產價值": round(val_sim, 2)})
        
        df_sim_res = pd.DataFrame(sim_data_list)
        final_sim_val = df_sim_res['資產價值'].iloc[-1]

        # 顯示統計指標
        c1, c2 = st.columns(2)
        with c1:
            st.metric(f"{years_to_sim}年後資產", f"{final_sim_val:,.0f} 萬")
        with c2:
            current_safe_withdraw = (final_sim_val * 0.04) / 12
            st.metric("屆時每月可提領額", f"{current_safe_withdraw:,.2f} 萬")

        # 繪製曲線圖
        fig_sim = px.area(df_sim_res, x='月份', y='資產價值', title="財富累積路徑 vs FIRE 目標線")
        fig_sim.add_hline(y=fire_target, line_dash="dash", line_color="red", annotation_text="FIRE 目標線")
        st.plotly_chart(fig_sim, use_container_width=True)

        # AI 診斷結論
        st.subheader("🤖 策略診斷結論")
        if final_sim_val >= fire_target:
            reach_m = df_sim_res[df_sim_res['資產價值'] >= fire_target]['月份'].iloc[0]
            st.success(f"🎉 恭喜！依照此計畫，您將在第 **{reach_m}** 個月（約 {reach_m//12} 年）達成財富自由。")
        else:
            gap_wan = fire_target - final_sim_val
            st.warning(f"目前計畫在 {years_to_sim} 年後尚有 **{gap_wan:,.0f} 萬** 的缺口。")
            st.info(f"💡 建議：若要準時達標，每月加碼需提升至 **{((fire_target - init_capital*((1+roi_annual/12)**months_sim)) / (((1+roi_annual/12)**months_sim-1)/(roi_annual/12))):.2f} 萬**。")